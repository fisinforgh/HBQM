...
#This script install HBQMSoftEdu
...
welcome=$(echo " "
	  echo "Welcome to HBQMSoftEdu Install"
	  echo " "
...
if [ $cont -eq 1 ]; then
	...
	echo "Please click CONTINUE to select HBQMSoftEdu Installation Folder"
	echo " "
	command -v $_ROOT
	$pathROOTEXE -q
	...
	echo "$pathROOTCONFIG"
	echo " ")
    echo "$infoINS1" | (zenity --text-info --title="HBQMSoftEdu install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install')
    if [ $? -eq 1 ]; then
	echo "Exit"
	exit 2
    fi   
    ...
    echo "$infoINS2" | (zenity --text-info --title="HBQMSoftEdu install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install')
    if [ $? -eq 1 ]; then
	echo "Exit"
	exit 2
    fi   
...
dirPATH=$(zenity --file-selection   --directory --title="HBQMSoftEdu install Choose a Installation Folder" --width=800 --height=400 )

(zenity --info --text="Please click CONTINUE to select Desktop Folder" --title="HBQMSoftEdu install choose Desktop Folder" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="HBQMSoftEdu install choose a Desktop Folder" --width=800 --height=400)

tarFILE="HBQMSoftEdu.tar"
pwd
echo "Installation folder: $dirPATH"
echo "Path to Desktop folder: $dirDESK"
echo "Downloading HBQM source files: ... "
echo "wget https://github.com/fisinforgh/HBQM/raw/refs/heads/main/$tarFILE -P $dirPATH"
echo "Wait..."
(wget -O $dirPATH/$tarFILE https://github.com/fisinforgh/HBQM/raw/refs/heads/main/$tarFILE) | zenity --progress --width=800 --height=400 --auto-close \
										     --title="HBQMSoftEdu Install" \
										     --text="Downloading HBQMSofEdu... Please wait ..." \
										     --percentage=10
echo " "
...
then
    (echo "Please click CONTINUE to install HBQMSoftEdu"
    echo "$tarFILE successfully downloaded!!") | (zenity --text-info --width=800 --height=400  --title="HBQMSoftEdu install" \
...
if [ -f "$FILE" ]; then
    rm $FILE
    ($pathROOTCLING -f SEHBQMDict.cxx -c SEHBQM.h SEHBQMLinkDef.h
     g++ -o SEHBQM SEHBQM.cxx  SEHBQMDict.cxx `$pathROOTCONFIG --cflags --glibs`) | zenity --progress --width=800 --height=400 --auto-close \
												    --title="HBQMSofEdu Install" \
												    --text="Compiling HBQMSoftEdu software" \
												    --percentage=10
else
    ($pathROOTCLING -f SEHBQMDict.cxx -c SEHBQM.h SEHBQMLinkDef.h
     g++ -o SEHBQM SEHBQM.cxx  SEHBQMDict.cxx `$pathROOTCONFIG --cflags --glibs`) | zenity --progress --width=800 --height=400 --auto-close \
												    --title="HBQMMSofEdu Install" \
												    --text="Compiling HBQMSofEdu software" \
												    --percentage=10
fi
...
if [ -f "$FILE" ]; then
    ...
    (zenity --info --text="HBQMSoftEdu executable compilation complete $FILE. \nPlease click CONTINUE to setup HBQMM executable globally and  desktop icon "\
	    --title="HBQMSoftEdu install" --ok-label='CONTINUE' --width=800 --height=400 )
else 
    echo "Something went WRONG. Please check requirements"
    echo "and try again"
    (zenity --info --text="Please check g++ compiler and try it again "\
	    --title="HBQMSoftEdu install ERROR" --ok-label='Exit install' --width=800 --height=400 )
    if [ $? -eq 0 ]; then
	echo "Exit"
	exit 2
    fi
...
echo "Path=$dirPATH/$nameDIR" >> "$deskFILE"     
echo "Icon=$dirPATH/$nameDIR/Icons/logoHBQM.png" >> "$deskFILE"
