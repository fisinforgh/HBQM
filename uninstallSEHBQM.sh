...
#This script UNINSTALL HBQMSoftEdu software
...
messageUNIS=$(echo "Welcome to the HBQMSoftEdu UNINSTALL wizard"
	      echo "You are about to uninstall  HBQMSoftEdu software"
	      echo "If you are completely sure, please clik on UNINSTALL"
	      )

echo "$messageUNIS" | (zenity --text-info --title="HBQMSoftEdu uninstall" --width=800 --height=400 --ok-label='UNINSTALL' --cancel-label='Exit')
if [ $? -eq 1 ]; then
    echo "Exit"
    exit 2
fi
...
(zenity --info --text="Please click CONTINUE to select \"Desktop\" Folder to UNISTALL HBQMSoftEdu desktop's icon" --title="HBQMSoftEdu UNINSTALL desktop icon" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="HBQMSoftEdu UNINSTALL choose a Desktop Folder" --width=800 --height=400)

manUNIS=$( echo "Remove HBQMSoftEdu software manually:"
	   ...
	   echo "# $nameDIR globally available executable "
...
hiddenFILE=".pathDIR_HBQMSoftEdu.txt"
dirNAME="HBQMSoftEdu"
